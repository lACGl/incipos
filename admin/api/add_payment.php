...
try {
    ...
    $stmt = $conn->prepare("
        SELECT b.*, 
               (SELECT SUM(o.odeme_tutari) FROM musteri_borc_odemeler o WHERE o.borc_id = b.borc_id) as odenen_tutar
    ...
    // Kalan borç hesapla
$odenen_tutar = $borc['odenen_tutar'] ?: 0;
$net_tutar = $borc['toplam_tutar'] - $borc['indirim_tutari']; 
...
$net_tutar = $borc['toplam_tutar'] - $borc['indirim_tutari']; // Net tutarı hesapla (indirim düşürülerek)
...
$stmt->execute([$odendi_mi, $borc_id]);
    ...
    echo json_encode([
        ...
        'odendi_mi' => $odendi_mi,
        'kalan_borc' => $borc['toplam_tutar'] - $yeni_odenen
    ]);
