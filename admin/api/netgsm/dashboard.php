...
if (isset($_POST['send_single_sms'])) {
    ...
    if (empty($phone) || empty($message)) {
        ...
        if ($result['success']) {
            $success_message = "SMS başarıyla gönderildi. İş ID: " . $result['job_id'];
        } else {
...
$stats_query = "SELECT 
                  COUNT(*) as total_sms,
                  SUM(CASE WHEN yanit REGEXP '^[0-9]{10,}$' THEN 1 ELSE 0 END) as successful_sms,
            ...
            <?php if ($balance['success']): ?>
                <div class="flex flex-col md:flex-row md:space-x-8">
                    <div class="mb-4 md:mb-0">
                        ...
                        <span class="font-bold"><?php echo number_format($balance['credit'], 2, ',', '.'); ?> ₺</span>
                    </div>
                    <div class="mb-4 md:mb-0">
                        <span class="text-gray-600">SMS Birim Adedi:</span>
                        <span class="font-bold"><?php echo number_format($balance['unit_credit'], 0, ',', '.'); ?> adet</span>
                    </div>
                    <div>
                        <span class="text-gray-600">Birim Fiyatı:</span>
                        <span class="font-bold"><?php echo number_format($balance['unit_price'], 4, ',', '.'); ?> ₺</span>
                    </div>
                </div>
            <?php else: ?>
                <p class="text-red-600">Bakiye bilgisi alınamadı: <?php echo $balance['message']; ?></p>
            <?php endif; ?>
        </div>
                            ...
                            <?php else: ?>
                                <?php foreach ($recent_sms as $sms): ?>
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 whitespace-nowrap text-sm">
                                            <?php 
                                                if (preg_match('/^[0-9]{10,}$/', $sms['yanit'])) {
        ...
        templateButtons.forEach(button => {
            button.addEventListener("click", function() {
                ...
                if (document.getElementById("message") && document.getElementById("message").value === "") {
                    document.getElementById("message").value = template;
                    document.getElementById("charCount").textContent = template.length;
                } else if (document.getElementById("bulk_message")) {
                    document.getElementById("bulk_message").value = template;
                    document.getElementById("bulkCharCount").textContent = template.length;
                }
            });
