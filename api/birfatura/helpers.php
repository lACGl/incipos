...
if (!function_exists('sendResponse')) {
    function sendResponse($data, $statusCode = 200) {
        ...
        // Daha önceden çıktı gönderilip gönderilmediğini kontrol et
        if (!headers_sent()) {
            // Header'ları burada ayarla
            http_response_code($statusCode);
            header('Content-Type: application/json');
            header('X-Content-Type-Options: nosniff');
        } else {
            // Header'lar zaten gönderilmiş, sadece log et
            file_put_contents($log_file, date('Y-m-d H:i:s') . " - Headers already sent, cannot modify headers\n", FILE_APPEND);
        }
        
        // ob_clean() yerine daha güvenli bir yaklaşım
        if (ob_get_level() > 0) {
            ob_clean(); // Sadece tampon varsa temizle
        }
