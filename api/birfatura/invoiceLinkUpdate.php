...
file_put_contents($log_file, date('Y-m-d H:i:s') . " - invoiceLinkUpdate endpoint çalıştırılıyor\n", FILE_APPEND);
...
file_put_contents($log_file, date('Y-m-d H:i:s') . " - Fatura linki güncelleme isteği: " . $jsonData . "\n", FILE_APPEND);

// Gelen verileri kontrol et ve logla
$faturaUrl = $data['faturaUrl'] ?? null;
$orderId = $data['orderId'] ?? null;
$faturaTarihi = $data['faturaTarihi'] ?? null;
$faturaNo = $data['faturaNo'] ?? null;

file_put_contents($log_file, date('Y-m-d H:i:s') . " - Sipariş ID: $orderId, Fatura URL: $faturaUrl\n", FILE_APPEND);
